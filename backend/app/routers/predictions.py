...
router = APIRouter()
allow_admin = RoleChecker(["CatAdmin"])

@router.post("/demand", response_model=DemandPredictionResponse)
def create_demand_prediction(prediction_in: DemandPredictionCreate, db: Session = Depends(get_db), current_user=Depends(allow_admin)):
...
def create_utilization_prediction(prediction_in: UtilizationPredictionCreate, db: Session = Depends(get_db), current_user=Depends(allow_admin)):
...
def create_maintenance_prediction(prediction_in: MaintenancePredictionCreate, db: Session = Depends(get_db), current_user=Depends(allow_admin)):
